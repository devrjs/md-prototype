/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import client from './src/http/client.ts'
import type { RequestConfig, ResponseErrorConfig } from './src/http/client.ts'
import type {
  PutProductsAttributesNamesIdMutationRequest,
  PutProductsAttributesNamesIdMutationResponse,
  PutProductsAttributesNamesIdPathParams,
  PutProductsAttributesNamesId401,
  PutProductsAttributesNamesId404,
} from '../types/PutProductsAttributesNamesId.ts'

function getPutProductsAttributesNamesIdUrl({ id }: { id: PutProductsAttributesNamesIdPathParams['id'] }) {
  return `http://localhost:3333/products/attributes/names/${id}` as const
}

/**
 * @description Esta rota permite atualizar as informações de um nome de atributo de produto específico. É necessário fornecer o ID do nome do atributo como parâmetro na URL e os novos dados no corpo da requisição. A atualização pode incluir a modificação do nome do atributo. O sistema verifica a existência do atributo e garante que não haja duplicidade de nomes antes de realizar a atualização.
 * @summary Altera um nome de atributo de produto específico.
 * {@link /products/attributes/names/:id}
 */
export async function putProductsAttributesNamesId(
  { id, data }: { id: PutProductsAttributesNamesIdPathParams['id']; data: PutProductsAttributesNamesIdMutationRequest },
  config: Partial<RequestConfig<PutProductsAttributesNamesIdMutationRequest>> & { client?: typeof client } = {},
) {
  const { client: request = client, ...requestConfig } = config

  const res = await request<
    PutProductsAttributesNamesIdMutationResponse,
    ResponseErrorConfig<PutProductsAttributesNamesId401 | PutProductsAttributesNamesId404>,
    PutProductsAttributesNamesIdMutationRequest
  >({ method: 'PUT', url: getPutProductsAttributesNamesIdUrl({ id }).toString(), data, ...requestConfig })
  return res.data
}